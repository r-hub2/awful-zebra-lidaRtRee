...
#' Convert gpstime
#'
#' Convert the gpstime from a LAS data into UTC date (leaps seconds 
#' considered until June 2025)
#' @param gpstime integer. extracted the gpstime field of a LAS file
#' @param type string. indicates the reference \code{AdjStdGPS} for Adjusted 
#' Standard GPS time or \code{Week} for GPS time of the week. The later is not 
#' implemented yet
#' @return A date of class \code{POSIXct}.
#' @examples
#' gpstime2Date(10^8)
#' @keywords internal
#' @export
gpstime2Date <- function(gpstime, type = "AdjStdGPS") {
  #
  # type should be indicated in las@header, but value is not always correct
  # Week type -> number of seconds since start of week, requires corresponding
  # date to convert (not in LAS file) -> currently not implemented
  # Adjusted Standard GPS time -> number of seconds since start of GPS time 
  # (minus 10^9 if date is after Sept. 14, 2011) -> ambiguity with Week type 
  # in this case
  # ref https://groups.google.com/g/lastools/c/_9TxnjoghGM
  # leap seconds taken into account until June, 2025)
  # 
  if (max(gpstime) > 3600*24*7 & type == "Week")
    warning("Type is week but values are larger than 3600*24*7")
  if (max(gpstime) <= 3600*24*7 & type == "AdjStdGPS")
    warning("Type is Adjusted Standard GPS time but values are smaller than 
          3600*24*7")
  # Adjusted Standard GPS time
  gps_epoch <- as.POSIXct("1980-01-06 00:00:00", tz = "UTC")
  # date no leap
  date_no_leap <- gps_epoch + gpstime + (type == "AdjStdGPS") * 10^9
  #
  # leap seconds table
  dates_leap <- as.POSIXct(c(
    "2016-12-31", "2015-06-30", "2012-06-30", 
    "2008-12-31", "2005-12-31", "1998-12-31", "1997-06-30",
    "1995-12-31", "1994-06-30", "1993-06-30", "1992-06-30",
    "1990-12-31", "1989-12-31", "1987-12-31", "1985-06-30",
    "1983-06-30", "1982-06-30", "1981-06-30"# , "1979-12-31", before gpstime
    # "1978-12-31", "1977-12-31", "1976-12-31", "1975-12-31",
    # "1974-12-31", "1973-12-31", "1972-12-31", "1972-06-30"
  ))
  # leap seconds before date
  leap_seconds <- sapply(date_no_leap, function(x) sum(x >= dates_leap))
  # adjusted date
  date_no_leap - leap_seconds
}
#-------------------------------------------------------------------------------
#' Load las_chablais3
#'
