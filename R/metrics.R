...
#' @param lm_stats character. Stats of the plane model by \code{\link{lm}} to be
#' returned, \code{adjR2_plane} for adjusted R square, \code{rmsr} for root mean
#' square of residuals.
...
#' lm_stats = c("adjR2_plane", "rmsr"))
#' @export
#'
terrain_points_metrics <- function(p, centre = NULL, r = NULL, lm_stats = "adjR2_plane") {
  ...
  if (nrow(p) <= 1) {
    output <- data.frame(altitude = as.numeric(NA), azimut_gr = as.numeric(NA), slope_gr = as.numeric(NA))
    if(is.element("adjR2_plane", lm_stats))
      output$adjR2_plane <- as.numeric(NA)
    if(is.element("rmsr", lm_stats))
      output$rmsr <- as.numeric(NA)
    return(output)
  }
  ...
  if (!is.na(slope) && abs(slope) > 0) {
    azimut <- ((pi / 2 - atan2(c, b)) * 400 / (2 * pi) + 200) %% 400
  } else {
    azimut <- as.numeric(NA)
  }
  ...
  if (!is.null(centre)) {
    ...
    ), resolution = 1, crs = sf::st_crs(p)$wkt)
    altitude <- try(as.numeric(terra::values(lidR::rasterize_terrain(p, dummyRaster, lidR::tin()))))
    if (inherits(altitude, "try-error")) altitude <- as.numeric(NA)
  } else {
  ...
  # output
  output <- data.frame(altitude = altitude, azimut_gr = azimut, slope_gr = slope)
  # stats
  if(is.element("adjR2_plane", lm_stats))
    output$adjR2_plane = summary(modlin)$adj.r.squared * 100
  if(is.element("rmsr", lm_stats))
    output$rmsr = sqrt(mean(stats::residuals(modlin)^2))
